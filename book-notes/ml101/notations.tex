...
With featurization, we can map input space $\mathbf{X}$ to $\mathbb{R}^d$, and the hypothesis space is of affine functions on feature space looks like
...
$R : [0, \infty) \to \mathbf{R}$ is the regularization term and $L : \mathbf{R}^n \to \mathbf{R}$ is the loss term.\footnote{Each inner product represents a (linear) prediction. Ridge regression and SVM are both of this form.}
