...
An \textbf{inner product space} (over reals) is a vector space $\mathbf{V}$ and an \textbf{inner product}, which is a mapping

$$
\langle . , . \rangle : \mathbf{V} \times \mathbf{V} \to \mathbf{R}
$$

This space has the properties that $\forall x, y, z \in \mathbf{V}$ and $a, b \in \mathbf{R}$,
\begin{itemize}
...
\textbf{Parallelogram law} states that a norm can be written in terms of an inner product on $\mathbf{V} ~iff~ \forall x, x' \in \mathbf{V}$,
...
\\
\\
Roughly, \textbf{projection onto a plane} can then be defined as for $x \in \mathbf{V}$, let $M$ be a subspace of inner product space $\mathbf{V}$, then $m_0$ is the projection of $x$ onto $M$ if $m_0 \in M$ and is the closest point to $x$ in $M$.
...
\\
\\
A \textbf{function} $f: \mathbb{R}^n \to \mathbb{R}$ is \textbf{convex} if $\text{dom}$ $f$ is a convex set and if for all $x, y \in \text{dom} ~ f$, and $0 \leq \theta \leq 1$, we have
...
Examples:
\begin{itemize}
  \item $f(x) = a x + b$ is both convex and concave on $\mathbb{R}$ for all $a, b \in \mathbb{R}$.
  \item $f(x) = |x|^p$ for $p \geq 1$ is convex on $\mathbb{R}$.
  \item $f(x) = e^{a x}$ for all $a$ is convex on $\mathbb{R}$.
  \item Every norm on $\mathbb{R}^n$ is convex.
  \item $f(x) = \text{max}\{ x \}$ is convex on $\mathbb{R}^n$.
\end{itemize}
...
Where $f_0$ is the objective function and $x \in \mathbb{R}^n$ are the optimization variables.
...
$$
\mathbf{V}(X - a) = \mathbf{V}(X)
$$

$$
\mathbf{E}(X - a) = \mathbf{E}(X) - a
$$

Say we are to use one value $m$ to represent a random variable $X$, s.t. mean square error is minimized.

\begin{align*}
\text{MSE}(m) &= \mathbf{E}[(X - m)^2] \\
              &= (\mathbf{E}[X - m])^2 + \mathbf{V}[X - m] \\
              &= (\mathbf{E}[X] - m)^2 + \mathbf{V}[X] \\
\end{align*}

$$
\derivative{\text{MSE}(m)}{m} = 2 m - 2 \mathbf{E}[X]
$$

Thus when $m = \mathbf{E}[X]$, $\derivative{\text{MSE}(m)}{m} = 0$.

So the value we should use is the expectation.
\\
\\
The expectation $\mu$ can be estimated from the sample ($y_1, \dots y_n$) mean.
...
$$
\mathbf{E}(U) = \mathbf{E}(\mathbf{E}(U | V))
$$


\textbf{Regression function}
...
$$
\argmin_{w \in \mathbb{R}^{d}, ~ b \in \mathbb{R}}{\frac{c}{n} \sum_{i = 1}^{n}{\text{max}(0, 1 - y_i (w^{T} x_i + b))} + \frac{1}{2} {\norm{w}_2^2}}
$$
...
Let $f : \mathbb{R}^d \to \mathbb{R}$ be a function.
A \textbf{level set} or \textbf{contour line} for the value $c$ is the set of points $x \in \mathbb{R}^d$ for which $f(x) = c$.

A \textbf{sublevel set} for the value $c$ is the set of points $x \in \mathbb{R}^d$ for which $f(x) \leq c$.

If $f : \mathbb{R}^d \to \mathbb{R}$ is \textbf{convex}, then sublevel sets are convex.
...
Suppose $f : \mathbb{R}^d \to \mathbb{R}$ is differentiable.
...
\\
\\
Now suppose $f : \mathbb{R}^d \to \mathbb{R}$ is convex and differentiable.

The linear approximation to $f$ at $x$ is a \textbf{global underestimator} of $f$.

$$
\forall ~ x, y \in \mathbb{R}^d, ~ ~ f(y) \geq f(x) + \bigtriangledown f(x)^{T} (y - x)
$$
...
A vector $g \in \mathbb{R}^d$ is a \textbf{subgradient} of $f : \mathbb{R}^d \to \mathbb{R}$ at $x$ if $\forall ~ z$,
...
As a reminder, for function $f : \mathbb{R}^d \to R$,
\begin{itemize}
  \item graph of function lives in $\mathbb{R}^{d + 1}$
  \item gradient, subgradient of $f$ live in $\mathbb{R}^d$, and
  \item contours, level sets, sublevel sets are in $\mathbb{R}^d$
\end{itemize}
...
Let $f : \mathbb{R}^d \to \mathbb{R}$ have a subgradient $g$ at $x_0$, hyperplane $H$ orthogonal to $g$ at $x_0$ must \textbf{support} the level set $S = \{ x \in \mathbb{R}^d | f(x) = f(x_0) \}$, meaning $H$ contains $x_0$ and all of $S$ lies on one side of $H$.
...
Mapping an input space $\mathbf{X}$ (sound wave, image, DNA sequence) to a vector $\mathbb{R}^d$ is called \textbf{feature extraction} or \textbf{featurization}.
...
With featurization, we can map input space $\mathbf{X} \subset \mathbb{R}^n$ to $\mathbb{R}^d$\footnote{And as it would seem, this optimization makes sense when $n < d$}, and the hypothesis space is of affine functions on feature space looks like

$$
\mathbf{H} = \{ x \to w^{T}\Phi{x} + b | w \in \mathbb{R}^d, b \in \mathbb{R} \}
$$
...
$$
\argmin_{w \in \mathbb{R}^d, b \in \mathbb{R}} (\frac{1}{2} w^{T} w + \frac{c}{n} \sum_{i = 1}^{n}{(1 - y_i [w^T \Phi(x_i) + b])_+}
$$
...
With this, we don't have to search over the feature space $w^*$ is in, $\mathbb{R}^d$, but rather having computed the $n$ $\Phi(x_i)$s, we search over that space of $n$ dimensions.
...
$$
\argmin_{\alpha \in \mathbb{R}^n} R(\sqrt{\alpha^T K \alpha}) + L(K \alpha)
$$
