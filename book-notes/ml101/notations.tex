...
By narrowing down the hypothesis space to only those functions satisfying being within a complexity measure.
...
$$
\argmin_{f \in \mathbf{F}}{\frac{1}{n} \sum_{i = 1}^{n}{l(f(x_i), y_i)}} ~ \text{s.t.} ~ \Omega(f) \leq r
$$

is Ivanov regularization, and

$$
\argmin_{f \in \mathbf{F}}{\frac{1}{n} \sum_{i = 1}^{n}{l(f(x_i), y_i)} + \lambda \Omega(f)}
$$
