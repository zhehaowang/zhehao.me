...
\textbf{Intuition}: LLN states that average is a consistent estimator of expectation when $n$ is large enough, CLT says how good it is.
...
\begin{itemize}
  \item Almost surely (a.s.) convergence: $T_n \overset{a.s.}{\underset{n \rightarrow \infty}{\longrightarrow} T}$ iff $\mathbb{P}[\{ \omega : T_n(\omega) \underset{n \rightarrow \infty}{\longrightarrow} T(\omega)\}] = 1$
  \item Convergence in probability: $T_n \overset{\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} T}$ iff $\mathbb{P}[|T_n - T| \geq \epsilon] \underset{n \rightarrow \infty}{\longrightarrow} 0, \forall \epsilon > 0$
  \item Convergence in $L^p$: $T_n \overset{(L^p)}{\underset{n \rightarrow \infty}{\longrightarrow} T}$ iff $\mathbb{P}[|T_n - T|^p] \underset{n \rightarrow \infty}{\longrightarrow} 0$
  \item Convergence in distribution $L^p$: $T_n \overset{(d)}{\underset{n \rightarrow \infty}{\longrightarrow} T}$ iff $\mathbb{P}[T_n \leq x] \underset{n \rightarrow \infty}{\longrightarrow} \mathbb{P}[T \leq x]$
\end{itemize}
...
$$
T_n \overset{a.s. / \mathbb{P} / (d)}{\underset{n \rightarrow \infty}{\longrightarrow} T} \implies f(T_n) \overset{a.s. / \mathbb{P} / (d)}{\underset{n \rightarrow \infty}{\longrightarrow} f(T)}
$$
...
If $U_n \overset{a.s./\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} U}$ and
$V_n \overset{a.s./\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} V}$, then

$$
V_n + U_n \overset{a.s./\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} V + U}
$$
...
$$
\sqrt{n} (Z_n - \theta) \overset{a.s./\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} \mathbf{N}(0, \sigma^2)}
$$
...
$$
\sqrt{n} (g(Z_n) - g(\theta)) \overset{a.s./\mathbb{P}}{\underset{n \rightarrow \infty}{\longrightarrow} \mathbf{N}(0, g'(\theta)^2 \sigma^2)}
$$

\subsection{Confidence interval}

Given observations $x_1, \dots, x_n$ and a \textbf{confidence level} $\gamma$, a valid \textbf{confidence interval} has a $\gamma$ probability of containing the true underlying parameter.
...
In general, the Degrees of Freedom of an estimate of a parameter are equal to the number of independent score that go into the estimate minus the number of parameters used as intermediate steps in the estimation of the parameter itself.
...
A \textbf{statistical experiment} is the pair $(E, \{ \mathbb{P}_theta \}_{\theta \in \Theta})$, where $E$ is a set, the \textbf{sample space} (the possible values my observations can take), and each of $P_theta$ is a probability distribution
...
Well specified means for an observation $X$, $\exists \theta \in \Theta$ where $X \sim \mathbb{P}(\theta)$. This is a strong assumption. ($X$ is just any one among the i.i.d $X_1, \dots X_n$).
...
The parameter $\theta$ is called \textbf{identified / identifiable} (in this model) iff the map $ \theta in \Theta \to \mathbb{P}_{\theta}$ is \textbf{injective}. I.e.
...
The \textbf{bias} of an estimator is how far the $\hat{\theta}$ it gets is from $\theta$: $\mathbf{E}[\hat{\theta}] - \theta$.
...
However being unbiased is not the only thing we are after: consider a Bernoulli distribution, for $n$ i.i.d data points, by linearality $\mathbf{E}[\bar{X_n}] = \frac{1}{n} \Sigma_{i=1}^{n}{\mathbf{E}[X_i]} = \mathbf{E}[X_1] = p$.
This is saying the bias of $n$ data points is as good as $1$ data point.

Hence we also have \textbf{quadratic risk} of $\hat{\theta}$, $\mathbf{E}[|\hat{\theta} - \theta|^2]$.
...
So going back to the Bernoulli example, $\mathbf{E}[\bar{X_n}]$ is consistent as $n \to \infty$, but $\mathbf{E}[X_1]$ is not.

This \textbf{quadratic risk} is the sum of bias and variance.

\begin{align*}
   \mathbf{E}[|\hat{\theta} - \theta|^2] &= \mathbf{E}[(\hat{\theta} - \mathbf{E}[\hat{\theta}] + \mathbf{E}[\hat{\theta}] - \theta)^2] \\
                                         &= \mathbf{E}[(\hat{\theta} - \mathbf{E}(\hat{\theta}))^2] + (\mathbf{E}[\hat{\theta}] - \theta)^2 \\
