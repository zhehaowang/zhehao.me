...
\begin{align*}
\mathbb{V}[X] &= \frac{1}{n} \sum_{i}{(x_i - \mathbb{E}[X])^2} \\
              &= \frac{1}{n} \sum_{i}{x_i^2 - 2 \mathbb{E}[X] \cdot x_i + \mathbb{E}[X]^2} \\
...
F(X, Y) = F(X) F(Y)
$$
Where $F(X)$, $F(Y)$ are cumulative distribution functions of $X$, $Y$; and $F(X, Y)$ is the join cumulative distribution function,
$$
