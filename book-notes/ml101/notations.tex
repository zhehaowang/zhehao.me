...
This function is the \textbf{regression function}, and
...
When causal relationship $X \to Y$ cannot be assumed, the noise random variable can be dependent on $X$, thus the general form $Y|X = \mu(X) + \eta(X)$.
...
\subsection{Ordinary least squares linear regression as smoothing}
